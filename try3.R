...
system.time(check_dupes(web=web6, old= old6, correction=1))
...
time_thing <- function(res){
  ...
  res <- res %>% mutate("y_n"= rep("yes",nrow(res)))
  for (i in 1:nrow(res)){
    #pull web time
    ...
    if((web_hour==old_start_hour) | (web_hour==old_end_hour)){res$match_type[i] = "Exact"}
        else if ((web_hour > old_start_hour) & (web_hour < old_end_hour)){res$match_type[i]  = "Range"}
        else {res$match_type[i]  = "No match"
        res$y_n[i]="no"}
  }
  #filter time_result by matches
  time_result <<- res %>% filter(y_n == "yes")
  write.csv(time_result, "test4_time_result.csv")
}

#call the function 
time_thing(res= final_result)
